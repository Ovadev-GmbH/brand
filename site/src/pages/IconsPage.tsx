 ...
 *  Lucide and Phosphor export each icon as Name and NameIcon; the Icon
 *  spelling is used. */
async function loadIcons(kind: "hugeicons" | "lucide" | "phosphor"): Promise<Icon[]> {
  if (kind === "phosphor") {
    const mod = await import("@phosphor-icons/react");
    return Object.entries(mod)
      /* Components only: IconContext and IconBase are not drawings. */
      .filter(([name, v]) => /^[A-Z]/.test(name) && name.endsWith("Icon") && name !== "IconBase" && (typeof v === "object" || typeof v === "function") && v !== null)
      .map(([name, Cmp]) => {
