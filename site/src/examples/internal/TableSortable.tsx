...
export default function TableSortable() {
  ...
  return (
    <Table ariaLabel="Badis">
      <thead>
        <tr>
          <Th sortKey="name" sort={sort} onSort={onSort}>Badi</Th>
          <Th>Kanton</Th>
          <Th num sortKey="visitors" sort={sort} onSort={onSort}>Besucher</Th>
        </tr>
      </thead>
      <tbody>
        {rows.map((r) => (
          <tr key={r.name}>
            <NameCell href="/internal/table" sub={r.canton === "ZH" ? "Zürichsee" : "Zugersee"}>{r.name}</NameCell>
            <Td>{r.canton}</Td>
