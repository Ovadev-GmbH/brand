...
export default {
  description:
    "Buttons that act on the same thing, joined into one control: a split action, an input with its button, a set of row actions. For options that stay pressed, use [Toggle Group](/ticketova/toggle-group).",
...
└── ButtonGroup
  ...
  sections: [
    ...
    {
      title: "Input",
      text: "An `Input` in the group takes the free width; the buttons keep theirs. See [Input](/ticketova/input).",
      example: "input",
    },
    {
      title: "Select",
      text: "A `SelectTrigger` in the group is as wide as its value unless it is given a width. See [Select](/ticketova/select).",
      example: "select",
    },
  ],
  api: [
