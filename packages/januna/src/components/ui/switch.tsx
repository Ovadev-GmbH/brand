...
function Switch({
  ...
  return (
    <SwitchPrimitive.Root
      ...
      data-size={size}
      className={cn(
        "peer group/switch relative inline-flex shrink-0 items-center rounded-pill border-2 transition-[color,background-color,border-color,box-shadow,transform] duration-fast ease-brand outline-none group-has-[:focus-visible]/field-label:ring-0 after:absolute after:-inset-x-3 after:-inset-y-2 focus-visible:ring-1 focus-visible:ring-brand-bright aria-invalid:ring-1 aria-invalid:ring-status-danger data-[size=default]:h-6 data-[size=default]:w-11 data-[size=sm]:h-4 data-[size=sm]:w-6 data-checked:border-brand data-checked:bg-brand group-has-[:focus-visible]/field-label:data-checked:border-brand data-unchecked:border-transparent data-unchecked:bg-gray-alpha-300 group-has-[:focus-visible]/field-label:data-unchecked:border-transparent data-disabled:cursor-not-allowed data-disabled:opacity-50",
        className
      )}
      {...props}
    >
