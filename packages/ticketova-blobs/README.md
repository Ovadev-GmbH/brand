...
Deterministic blob avatars for TICKETOVA, after [Blobatar](https://github.com/Alain00/blobatar) (MIT). Each one is a flat silhouette with two strokes for eyes. Everything is drawn from TICKETOVA's grey ramp, and the shapes include items from the ticket counter: a ticket, a postage stamp, a wristband and a coin. This is an independent implementation; no Blobatar code is included.
...
- **Traits:** 10 shapes, 6 tones (`--tova-gray-400` to `-1000`), plus continuous tilt, stretch and eye geometry. Every trait reads its own hash key, so adding a trait later does not change existing blobs. Any trait can be pinned with `traits`.
