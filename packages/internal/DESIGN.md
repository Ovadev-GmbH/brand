...
Screens and screen parts composed from the components, so a whole page starts from one import. Each is shown with every case at https://intern.ova.dev/brand/internal/<slug>, and as Markdown at the same path with `.md`. Start a screen from a block where one fits; compose from components where none does.

```tsx
import { AppShell, AuthLayout } from "@ovadev-gmbh/ui-internal/blocks";
```

- App Shell (`app-shell`): AppShell, AppShellHeader, AppShellContent
- Auth (`auth`): AuthLayout, AuthCard, LoginForm, SignupForm, ForgotPasswordForm, OtpForm
