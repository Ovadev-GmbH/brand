...
const alertVariants = cva(
  "group/alert relative grid w-full gap-0.5 rounded-regular border px-3 py-2.5 text-left text-copy-14 has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2.5 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
      ...
      size: {
        default: "",
        sm: "flex items-center gap-2.5 px-3 py-1.5 text-label-13 has-data-[slot=alert-action]:pr-3 *:[svg]:translate-y-0 *:[svg:not([class*='size-'])]:size-3.5 *:data-[slot=alert-title]:text-label-13 *:data-[slot=alert-description]:text-label-13 *:data-[slot=alert-action]:static *:data-[slot=alert-action]:ml-auto",
      },
    },
    defaultVariants: {
      variant: "default",
...
function AlertDescription({
  ...
  return (
    <div
      ...
      className={cn(
        "text-copy-14 text-balance text-content-secondary md:text-pretty [&_a]:underline [&_a]:underline-offset-3 [&_a]:hover:text-content-primary [&_p:not(:last-child)]:mb-2",
        className
      )}
      {...props}
    />
